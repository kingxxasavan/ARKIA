...
- **Chat history** — conversations sync to your account (Realtime Database)
...
The Firebase config is embedded in the client (this is normal — Firebase web keys are public). Security comes from Firebase Auth + the Realtime Database rules below.
...
2. **Realtime Database → Create Database** (free on the Spark plan — no card needed). Pick a location
   and start in **locked mode**. Copy the **database URL** shown at the top (e.g.
   `https://arikia-default-rtdb.firebaseio.com` or `…-default-rtdb.<region>.firebasedatabase.app`) and
   paste it into the `databaseURL` field of `firebaseConfig` in **both** `index.html` and `app.html`
   (and the `public/` copies), and into `FIREBASE_DB_URL` on the server if you use server-side prompt lock.
   Then open the **Rules** tab → paste and **Publish**:
   ```json
   {
     "rules": {
       ".read": false,
       ".write": false,
       "hub":    { ".read": true, ".write": "auth != null && (auth.token.email == 'kingasd8970@gmail.com' || auth.token.email == 'fanegf837@gmail.com' || auth.token.email == 'kingfan837@gmail.com')" },
       "config": { ".read": true, ".write": "auth != null && (auth.token.email == 'kingasd8970@gmail.com' || auth.token.email == 'fanegf837@gmail.com' || auth.token.email == 'kingfan837@gmail.com')" },
       "limits": { ".read": true, ".write": "auth != null && (auth.token.email == 'kingasd8970@gmail.com' || auth.token.email == 'fanegf837@gmail.com' || auth.token.email == 'kingfan837@gmail.com')" },
       "memories": { "$uid": { ".read": "auth != null && auth.uid == $uid", ".write": "auth != null && auth.uid == $uid" } },
       "chats":    { "$uid": { ".read": "auth != null && auth.uid == $uid", ".write": "auth != null && auth.uid == $uid" } },
       "usage":    { "$uid": { ".read": "auth != null && auth.uid == $uid", ".write": "auth != null && auth.uid == $uid" } }
     }
   }
   ```
...
> **Why Realtime Database (not Firestore)?** It's on the free Spark plan with no billing/card,
> and the data layer lives behind `window.HUB_FB` / `window.ARIA_FB`, so the rest of the app is
> unchanged. Storage paths: `hub/main` (directory), `config/app` (site config), `memories/<uid>`,
> `chats/<uid>`, `usage/<uid>`, `limits/<uid>`.
...
2. **Settings → Environment Variables**, then redeploy:
   ...
   | `FIREBASE_DB_URL` | *(optional)* let the server read the **admin panel's** locked prompt from Realtime Database (public read, no key needed) |
...
> **Panel-controlled server lock.** To let the server read the admin panel's locked prompt, set
> `FIREBASE_DB_URL` to your Realtime Database URL — public RTDB reads need no key, so this works from
> any server (unlike Firestore, whose web key is referrer-restricted). Or skip it and use the
> `LOCK_SYSTEM_PROMPT` / `LOCKED_SYSTEM_PROMPT` env vars.
...
- The landing page is a **curated directory**: the website list lives in Realtime Database at `hub/main` and is **public to read** but **only admins can edit it**. Sign in via the **Sign in** button on the homepage with an admin account to reveal the add/edit/remove controls; everyone else sees a read-only directory. Sites are grouped into **categories** and each entry carries an optional **image** (defaults to the site's favicon).
- **Admins can reorder categories** (drag a category header, or use the ▲/▼ buttons), **rename a category** (✎ — re-tags all its sites at once), and **edit any page text** — eyebrow, hero tagline, the Who/What/Why cards, the directory heading and footer — via the **Edit text** button. Order is stored as `catOrder` and text as `content` in `config/app`. The brand name/title still comes from the chat app's admin panel.
- **Draft + preview workflow:** admin edits (sites, categories, page text) are kept as a **local draft** and are *not* live until you press **Push live**. A bar shows when you have unpublished changes, with **Preview** (see the page exactly as visitors do) and **Discard**. Because nothing reaches the database until you push, a broken edit can't take down the live site; if a push fails, your draft is kept. The draft survives a page reload.
...
- **Admin accounts** are defined in three places that must stay in sync: `ADMIN_EMAILS` in `index.html`/`public/index.html`, `ADMIN_EMAILS` in `app.html`/`public/app.html`, and the email list in the Realtime Database rules. Current admins: `kingasd8970@gmail.com`, `fanegf837@gmail.com`, `kingfan837@gmail.com`.
- **Site-wide admin controls** live in the chat app under **Settings → Admin controls** (visible only to admins) and are stored in Realtime Database at `config/app` (public read, admin-only write). From there an admin can, for everyone: change the **active model**, post an **announcement popup**, set a **locked system prompt** (when locked, every user's prompt is enforced and their own System Prompt box becomes read-only until the admin presses Save), set the **app/homepage name and tagline**, and set the **default theme & accent color**.
- **Locked-prompt enforcement** has two layers. The panel toggle enforces the prompt in the browser (read-only box). For true server-side enforcement that a user can't bypass, the `/api/ollama` proxy rewrites the system message on every chat request — driven by either host env vars (`LOCK_SYSTEM_PROMPT=1` + `LOCKED_SYSTEM_PROMPT=...`, always reliable) or, if you set `FIREBASE_DB_URL`, the panel's saved prompt read from Realtime Database (public read, no key needed).
- **Web search** runs through the server (`/api/search`, scraping DuckDuckGo's HTML/Lite endpoints) because browsers can't call search engines directly (CORS). On a static host with no server, search is unavailable and ARIA answers from its own knowledge. Both the homepage and the chat app subscribe to this config live. The panel has the same **Preview → Push live → Discard** flow as the homepage: **Preview** applies your edits in your tab only (brand, theme/accent, model badge, locked-prompt state, announcement) so you can see them before **Push live** writes them to the database for everyone; **Discard** reverts to the live values. (This preview is in-tab only and not saved across reloads.) Per-user message limits (the "ban"/quota controls) remain at `limits/{uid}` and admins bypass them.
